...
RESPUESTA
Es negativa. Cuanto mayor sea el tamaño del motor del coche en litros, menor será la eficiencia de combustible por galón y, por tanto, el coche consumirá más combustible a medida cuanto mayor sea el tamaño del motor.
---
...
RESPUESTA
La variable `class` es de tipo 'character'
Se comportan los estéticos para variables continuas de manera similar que para variables catégoricas.
...
```{r}
ggplot(data=mpg) + geom_point(mapping = aes(x = displ, y = hwy, color = manufacturer, size=cty))
```

3) ¿Qué ocurre cuando intentamos diferenciar los puntos por forma (`shape`) en base a una variable continua (e.g. `cty`)?
RESPUESTA
No hay suficientes formas en el paquete para plasmar todos los datos segun la forma
Se borran 137 filas que contienen valores perdidos o no almacenados en la variable, valores desconocidos.

```{r}
ggplot(data=mpg) + geom_point(mapping = aes(x = displ, y = hwy, color=class, shape=factor(cty)))
...
```{r}
#1)
vuel_sin_hora <- filter(flights, is.na(dep_time))
#vuel_sin_hora
#length(vuel_sin_hora$dep_time)
#RESPUESTA
#8255 vuelos sin hora de salida
#2)
distance_sorted <- arrange(flights, desc(distance))
distance_sorted$distance
vuelos_max_distance <- filter(distance_sorted, distance==max(distance))
#length(vuelos_max_distance$flight)
vuelos_min_distance <- filter(distance_sorted, distance==min(distance))
#length(vuelos_min_distance$flight)
#RESPUESTA
#Los de mayor trayecto han sido aquellos vuelos que han recorrido 4963km que son 342 vuelos.
#Los de menor trayecto han sido aquellos vuelos que han recorrido km que solo es 1  vuelo.
```
...
RESPUESTA:
read_tsv() porque es la función por defecto que separa por tabulaciones.
...
```{r}
?write_csv
write_csv(flights, tempfile(), na = "NA", append = FALSE)
...
```{r}
?unite
fl <- unite(flights, 'date', c('year', 'month', 'day'), sep='/')
fl
```
